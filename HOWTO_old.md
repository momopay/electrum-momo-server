How to run your own Electrum momo server
========================================
...
Electrum momo server on Linux. It is structured as a series of steps you need to
...
The most up-to date version of this document is available at:
    ...
    https://github.com/chinafreak/electrum-momo-server/blob/master/HOWTO.md
...
document, we assume that user is called 'momo'). We also assume the
momo user has sudo rights, so we use `$ sudo command` when we need to.
...
you also need to run momod and keep a copy of the full blockchain,
which is roughly 55 GB (February 2016). Ideally you have a machine with 16 GB of RAM
and an equal amount of swap. If you have ~2 GB of RAM make sure you limit momod 
...
only leaving spare cycles for momod. Fast single core CPU power helps for the initial 
...
### Step 1. Create a user for running momod and Electrum momo server
...
### Step 2. Download momod

Recommend downloading latest version directly from momo.org

    $ sudo apt-get install make g++ python-leveldb libboost-all-dev libssl-dev libdb++-dev pkg-config libevent-dev
    $ sudo su - momo
    $ cd ~/src && wget https://www.momo.org/binaries/momo-0.12.0.57-linux64.tar.gz
    $ sha256sum momo-0.12.0.57-linux64.tar.gz | grep 2a29b529c56d2ba41e28dfb20872861d6b48bdfe4fb327bfd2273123b38139aa
    $ tar xfz momo-0.12.0.57-linux64.tar.gz
    $ cd momo-0.12.0/bin
    $ cp -a momod momo-cli momo-tx ~/bin

### Step 3. Configure and start momod

In order to allow Electrum momo to "talk" to `momod`, we need to set up an RPC
username and password for `momod`. We will then start `momod` and
wait for it to complete downloading the blockchain.

    $ mkdir ~/.momo
    $ $EDITOR ~/.momo/momo.conf
...
If you have an existing installation of momod and have not previously
...
Allow some time to pass for `momod` to connect to the network and start
downloading blocks. You can check its progress by running:

    $ momo-cli getblockchaininfo

Before starting the Electrum momo server your momod should have processed all
blocks and caught up to the current height of the network (not just the headers).
You should also set up your system to automatically start momod at boot
time, running as the 'momo' user. Check your system documentation to
find out the best way to do this.

### Step 4. Download and install Electrum momo server

We will download the latest git snapshot for Electrum to configure and install it:

    $ cd ~
    $ git clone https://github.com/Chinafreak/electrum-momo-server.git
    $ cd electrum-momo-server
...
### Optional Step 5: Install Electrum momo dependencies manually

Electrum momo server depends on various standard Python libraries and leveldb. These will usually be
...
Electrum momo server uses leveldb to store transactions. You can choose
...
and further instructions from the Electrum momo full archival server foundry at a later time.
...
server.key corresponds to `ssl_keyfile=` in your Electrum momo server config.
...
### Step 9. Configure Electrum momo server

Electrum momo reads a config file (/etc/electrum-momo.conf) when starting up. This
file includes the database setup, momod RPC setup, and a few other
options.

The "configure" script listed above will create a config file at /etc/electrum-momo.conf
...
Electrum momo server currently needs quite a few file handles to use leveldb. It also requires
file handles for each connection made to the server. It's good practice to increase the
open files limit to 64k.

The "configure" script will take care of this and ask you to create a user for running electrum-momo-server.
If you're using the user `momo` to run electrum and have added it as shown in this document, run
the following code to add the limits to your /etc/security/limits.conf:

     echo "momo hard nofile 65536" >> /etc/security/limits.conf
     echo "momo soft nofile 65536" >> /etc/security/limits.conf
...
Check if the limits are changed either by logging with the user configured to run Electrum momo server as. Example:
...
Or if you use sudo and the user is added to sudoers group:
    ...
    sudo -u momo -i ulimit -n


Two more things for you to consider:

1. To increase privacy of transactions going through your server
   you may want to close momod for incoming connections and connect outbound only. Most servers do run
   full nodes with open incoming connections though.

2. Consider restarting momod (together with electrum-server) on a weekly basis to clear out unconfirmed
   transactions from the local the memory pool which did not propagate over the network.

### Step 11. (Finally!) Run Electrum momo server

The magic moment has come: you can now start your Electrum momo server as root (it will su to your unprivileged user):
...
If you want to stop Electrum momo server, use the 'stop' command:
...
### Step 12. Test the Electrum momo server
...
current server and connect to your new Electrum momo server. You should see your
addresses and transactions history. You can see the number of blocks and
response time in the server selection window. You should send/receive some
momos to confirm that everything is working properly.
...
If you're operating a public Electrum momo server please subscribe
