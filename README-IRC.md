IRC is used by Electrum momo server to find 'peers' - other Electrum momo servers. The
current list can be seen by running:

  electrum-momo-server peers

The following config file options are used by the IRC part of Electrum momo server:
...
the Electrum momo server will run in private mode (default). In private
mode, `electrum-momo-server peers` will always return an empty list.

`host` is a fully-qualified domain name (FQDN) of your Electrum momo
...
your Electrum momo server instead of `host`. It is used as part of the name
...
composing the IRC nickname to identify your server on #electrum-momo .
